...
/**
 * A country the user can pick for login/registration. [dialCode] is the international calling
 * code the Tuya SDK expects as its "countryCode" parameter (e.g. "62" for Indonesia).
 */
data class Country(
    val isoCode: String,
    val dialCode: String,
...
object Countries {
    ...
    private val DIAL_CODES: Map<String, String> = mapOf(
        ...
        "PL" to "48", "PT" to "351", "PR" to "1", "QA" to "974", "RO" to "40",
        "RU" to "7", "RW" to "250", "KN" to "1", "LC" to "1", "VC" to "1",
        "WS" to "685", "SM" to "378", "ST" to "239", "SA" to "966", "SN" to "221",
        "RS" to "381", "SC" to "248", "SL" to "232", "SG" to "65", "SK" to "421",
        "SI" to "386", "SB" to "677", "SO" to "252", "ZA" to "27", "KR" to "82",
        "SS" to "211", "ES" to "34", "LK" to "94", "SD" to "249", "SR" to "597",
        "SE" to "46", "CH" to "41", "SY" to "963", "TW" to "886", "TJ" to "992",
        "TZ" to "255", "TH" to "66", "TL" to "670", "TG" to "228", "TO" to "676",
        "TT" to "1", "TN" to "216", "TR" to "90", "TM" to "993", "TV" to "688",
        "UG" to "256", "UA" to "380", "AE" to "971", "GB" to "44", "US" to "1",
        "UY" to "598", "UZ" to "998", "VU" to "678", "VA" to "39", "VE" to "58",
        "VN" to "84", "YE" to "967", "ZM" to "260", "ZW" to "263"
    )

    /** All supported countries, sorted by display name. */
    val ALL: List<Country> = DIAL_CODES.map { (iso, dial) ->
        val name = Locale("", iso).getDisplayCountry(Locale.ENGLISH).ifBlank { iso }
        Country(iso, dial, name)
    }.sortedBy { it.name }

    /** Sensible fallback when nothing can be detected. */
    val DEFAULT: Country = fromIso("US") ?: ALL.first()

    fun fromIso(isoCode: String): Country? =
        ALL.firstOrNull { it.isoCode.equals(isoCode, ignoreCase = true) }

    /**
     * Best-effort guess of the device's country **without any runtime permission**. The current
     * time zone is tried first (it reflects where the device physically is and works on emulators),
     * then the SIM/mobile-network country, then the device locale.
     */
    fun detect(context: Context): Country {
        val candidates = buildList {
            // Time zone -> region, e.g. "Asia/Jakarta" -> "ID" (ICU, API 24+, no permission).
            runCatching { android.icu.util.TimeZone.getRegion(TimeZone.getDefault().id) }.getOrNull()?.let { add(it) }
            (context.getSystemService(Context.TELEPHONY_SERVICE) as? TelephonyManager)?.let { tm ->
                add(tm.simCountryIso)
                add(tm.networkCountryIso)
            }
            add(context.resources.configuration.locales[0].country)
        }
        return candidates
            .firstNotNullOfOrNull { raw ->
                raw?.trim()?.takeIf { it.length == 2 }?.let { fromIso(it) }
            } ?: DEFAULT
    }
}
